...
/*global Blob */
/*members Kepixel, Kepixel, encodeURIComponent, decodeURIComponent, getElementsByTagName,
    shift, unshift, kepixelAsyncInit, kepixelAsyncInit, kepixelPluginAsyncInit , frameElement, self, hasFocus,
    createElement, appendChild, characterSet, charset, all, kepixel_log, AnalyticsTracker,
    ...
    initialized, hook, getHook, resetUserId, getVisitorId, getVisitorInfo, setUserId, getUserId, setAppId, getAppId, getTrackerUrl, appendToTrackingUrl, getRequest, addPlugin,
    ...
    setReferrerUrl, setCustomUrl, setAPIUrl, setDocumentTitle, setPageViewId, getPageViewId, getKepixelUrl, getKepixelUrl, getCurrentUrl,
    ...
    trackVisibleContentImpressions, isTrackOnlyVisibleContentEnabled, port, isUrlToCurrentDomain, kepixelTrackers,
    ...
    sort, slice, stringify, test, toJSON, toString, valueOf, objectToJSON, removeAllAsyncTrackersButFirst,
...
/*global Kepixel:true */
/*global Kepixel:true */
/*members addPlugin, getTracker, getAsyncTracker, getAsyncTrackers, trigger, on, off, retryMissedPluginCalls,
...
// Kepixel singleton and namespace
if (typeof window.Kepixel !== 'object') {
    window.Kepixel = window.Kepixel = (function () {
        'use strict';
        ...
        function apply() {
            ...
            for (i = 0; i < arguments.length; i += 1) {
                ...
                if (isStaticPluginCall) {
                    ...
                    if ('object' === typeof Kepixel[context] && 'function' === typeof Kepixel[context][f]) {
                        Kepixel[context][f].apply(Kepixel[context], parameterArray);
                    } else if (trackerCall) {
                        // we try to call that method again later as the plugin might not be loaded yet
                        // a plugin can call "Kepixel.retryMissedPluginCalls();" once it has been loaded and then the
                        // method call to "Kepixel[context][f]" may be executed
                    ...
                    for (j = 0; j < asyncTrackers.length; j++) {
                        if (isString(f)) {
                            ...
                            if (context[f]) {
                                ...
                                var message = 'The method \'' + f + '\' was not found in "_paq" variable.  Please have a look at the Kepixel tracker documentation: https://kepixel.com';
        ...
        var content = {
            ...
            CONTENT_CLASS: 'kepixelTrackContent',
            LEGACY_CONTENT_CLASS: 'kepixelTrackContent',
            CONTENT_NAME_ATTR: 'data-content-name',
            CONTENT_PIECE_ATTR: 'data-content-piece',
            CONTENT_PIECE_CLASS: 'kepixelContentPiece',
            LEGACY_CONTENT_PIECE_CLASS: 'kepixelContentPiece',
            CONTENT_TARGET_ATTR: 'data-content-target',
            CONTENT_TARGET_CLASS: 'kepixelContentTarget',
            LEGACY_CONTENT_TARGET_CLASS: 'kepixelContentTarget',
            CONTENT_IGNOREINTERACTION_ATTR: 'data-content-ignoreinteraction',
            CONTENT_IGNOREINTERACTION_CLASS: 'kepixelContentIgnoreInteraction',
            LEGACY_CONTENT_IGNOREINTERACTION_CLASS: 'kepixelContentIgnoreInteraction',
            location: undefined,
        ...
        /*
         * Kepixel Tracker class
         *
         * trackerAppId are optional arguments to the constructor
         *
         * See: Tracker.setAppId()
         */
        function Tracker(appId) {
            ...
            var
                ...
                // constants
                CONSENT_COOKIE_NAME = 'kpx_consent',
                COOKIE_CONSENT_COOKIE_NAME = 'kpx_cookie_consent',
                CONSENT_REMOVED_COOKIE_NAME = 'kpx_consent_removed',
                ...
                // Tracker URL
                configTrackerUrl = 'https://edge.kepixel.com',
                ...
                // Campaign names
                configCampaignNameParameters = [ 'pk_campaign', 'mtm_campaign', 'kepixel_campaign', 'kepixel_campaign', 'utm_campaign', 'utm_source', 'utm_medium' ],

                // Campaign keywords
                configCampaignKeywordParameters = [ 'pk_kwd', 'mtm_kwd', 'kepixel_kwd', 'kepixel_kwd', 'utm_term' ],
                ...
                configCampaignKnownParameters = [
                    'mtm_campaign', 'kepixel_campaign', 'mtm_cpn', 'pk_campaign', 'kepixel_campaign', 'pk_cpn', 'utm_campaign', // campaign name
                    'mtm_keyword', 'kepixel_kwd', 'mtm_kwd', 'pk_keyword', 'kepixel_kwd', 'pk_kwd', 'utm_term', // campaign keyword
                ...
                // An initial list of known referrers that are sending unexpected/unwanted campaign parameters. Kepixel will ignore such campaigns if the referring URL matches one of the hosts defined below.
                ...
                // detect this 100% correct for an iframe so whenever Kepixel is loaded inside an iframe we presume
                // the window had focus at least once.
                hadWindowFocusAtLeastOnce = isInsideAnIframe(),
                timeWindowLastFocused = null,

                // Timestamp of last tracker request sent to Kepixel
                ...
                // or unless a kepixel user explicitly requires consent (via requireConsent())
            ...
            function purify(url) {
                ...
                // we need to remove this parameter here, they wouldn't be removed in Kepixel tracker otherwise eg
            ...
            /*
             * Send image request to Kepixel server using GET.
            ...
            /*
             * POST request to Kepixel server using XMLHttpRequest.
             */
            ...
            function setUpHeartBeat() {
                ...
                Kepixel.addPlugin('HeartBeat' + coreHeartBeatCounter, {
                    unload: function () {
            ...
            function getCookieName(baseName) {
                // NOTE: If the cookie name is changed, we must also update the KepixelTracker.php which
                // will attempt to discover first party cookies. eg. See the PHP Client method getVisitorId()
                return configCookieNamePrefix + baseName + '.' + configTrackerAppId + '.' + domainHash;
            }
            ...
            function setVisitorIdCookie(visitorIdCookieValues) {
                ...
                if(!configTrackerAppId) {
                    // when called before App ID was set
                    return;
                }
            ...
            function appendAvailablePerformanceMetrics(request) {
                ...
                // note: there might be negative values because of browser bugs see https://github.com/kepixel-org/kepixel/pull/16516 in this case we ignore the values
                var timings = '';
            ...
            /**
             * Returns the URL to call kepixel.php,
            ...
            function getRequest(request, customData, pluginMethod) {
                ...
                // do not send charset if it is utf8 since it's assumed by default in Kepixel
                ...
                request += '&appid=' + configTrackerAppId +
                    '&rec=1' +
            ...
            function getClassesRegExp(configClasses, defaultClass) {
                var i,
                    classesRegExp = '(^| )(kepixel[_-]' + defaultClass + '|kepixel[_-]' + defaultClass;
            ...
            function isLinkToDifferentDomainButSameKepixelWebsite(element)
            {
            ...
            function processClick(sourceElement) {
                ...
                if (crossDomainTrackingEnabled) {
                    ...
                    if(isLinkToDifferentDomainButSameKepixelWebsite(sourceElement)) {
            ...
            var requestQueue = {
                ...
                push: function (requestUrl) {
                    ...
                    if (!Object.prototype.hasOwnProperty.call(plugins, trackerQueueId)) {
                        ...
                        // Kepixel.addPlugin might not be defined at this point, we add the plugin directly also to make
                        // JSLint happy.
             ...
             *   1) Call windowAlias.JSON.stringify(kepixelTracker.getAttributionInfo())
            ...
            /**
             * Returns the Kepixel tracking URL
             * @returns {string}
             */
            ...
            /**
             * Returns the Kepixel server URL.
             *
             * @returns {string}
             */
            this.getKepixelUrl = function () {
                if (configApiUrl) {
                    return configApiUrl;
                }

                var trackerUrl = this.getTrackerUrl();
                ...
                if (stringEndsWith(trackerUrl, 'kepixel.php')) {
                    // if eg without domain or path "kepixel.php" => ''
                    trackerUrl = removeCharactersFromEndOfString(trackerUrl, 'kepixel.php'.length);
                } else if (stringEndsWith(trackerUrl, 'kepixel.php')) {
                    // if eg without domain or path "kepixel.php" => ''
                    trackerUrl = removeCharactersFromEndOfString(trackerUrl, 'kepixel.php'.length);
                } else if (stringEndsWith(trackerUrl, '.php')) {
                    // if eg http://www.example.com/js/kepixel.php => http://www.example.com/js/
                ...
                // if eg http://www.example.com/js/ => http://www.example.com/ (when not minified Kepixel JS loaded)
            ...
            /**
             * Appends the specified query string to the kepixel.php?... Tracking API URL
             *
            ...
            /**
             * Set array of domains to be treated as local. Also supports path, eg '.kepixel.com/subsite1'. In this
             * case all links that don't go to '*.kepixel.com/subsite1/ *' would be treated as outlinks.
             * For example a link to 'kepixel.com/' or 'kepixel.com/subsite2' both would be treated as outlinks.
             *
             * Also supports page wildcard, eg 'kepixel.com/index*'. In this case all links
             * that don't go to kepixel.com/index* would be treated as outlinks.
             *
             ...
             * feature, for Kepixel JS Tracker to work correctly in all cases, one needs to specify all hosts
             * manually.
             *
            ...
            this.setDomains = function (hostsAlias) {
                ...
                if (!hasDomainAliasAlready) {
                    /**
                     * eg if domainAlias = 'kepixel.com' and someone set hostsAlias = ['kepixel.com/foo'] then we should
                     * not add kepixel.com as it would increase the allowed scope.
                     */
            ...
            /**
             * Set array of domains to be excluded as referrer. Also supports path, eg '.kepixel.com/subsite1'. In this
             * case all referrers that don't match '*.kepixel.com/subsite1/ *' would still be used as referrer.
             * For example 'kepixel.com/' or 'kepixel.com/subsite2' would both be used as referrer.
             *
             * Also supports page wildcard, eg 'kepixel.com/index*'. In this case all referrers
             * that don't match kepixel.com/index* would still be treated as referrer.
             *
             ...
             *
             * On the different domain, the Kepixel tracker will recognize the set visitorId from the URL parameter and
             ...
             * Eg:
             *
             * var url = 'http://myotherdomain.com/?' + kepixelTracker.getCrossDomainLinkingUrlParameter();
            ...
            /**
             * Removed since Kepixel 4
             * @param generationTime
             */
            this.setGenerationTimeMs = function(generationTime) {
                logConsoleError('setGenerationTimeMs is no longer supported since Kepixel 4. The call will be ignored. The replacement is setPagePerformanceTiming.');
            };
            ...
            /**
             * Set the URL of the Kepixel API. It is used for Page Overlay.
             ...
             * Note: this can be done in the Kepixel>Settings>Websites on a per-website basis
             *
             * @deprecated
            ...
            this.setCookieConsentGiven = function () {
                if (configCookiesDisabled && !configDoNotTrack) {
                    ...
                    if (configTrackerAppId && hasSentTrackingRequestYet) {
                        setVisitorIdCookie();

                        // sets attribution cookie, and updates visitorId in the backend
                        // because hasSentTrackingRequestYet=true we assume there might not be another tracking
                        // request within this page view so we trigger one ourselves.
                        // if no tracking request has been sent yet, we don't set the attribution cookie cause Kepixel
             ...
             * a cookie named "kpx_cookie_consent". You can optionally define the lifetime of that cookie in hours
             * using a parameter.
             *
             ...
             * When clicked, Kepixel will log the click automatically.
             *
             ...
             *
             * If you change the DOM of your website or web application Kepixel will automatically detect links
             ...
             * recommend to include the Kepixel JavaScript tracker in the HTML as early as possible.
             ...
             * class `kepixelContentIgnoreInteraction` or the attribute `data-content-ignoreinteraction`.
             *
             * @param {Element} domNode  This element itself or any of its parent elements has to be a content block
             *                         element. Meaning one of those has to have a `kepixelTrackContent` CSS class or
            ...
            /**
             * Validates user data to ensure it's an object with at least one of the required properties
             *
            ...
            function validateUserData(user_data) {
                if (!user_data || typeof user_data !== 'object' || Array.isArray(user_data)) {
                    console.warn('Anubis: user_data parameter should be an object');
                    return false;
                }

                if (!user_data.email && !user_data.phone && !user_data.name && !user_data.id) {
                    console.warn('Anubis: user_data object should have at least one of these properties: email, phone, name, id');
                    return false;
                }
            ...
            /**
             * Checks if user data has email, phone, or id and calls setUserId if it does
             *
             * @param {Object} user_data - User data to check
             */
            function checkAndSetUserId(user_data) {
                if (!user_data || typeof user_data !== 'object' || Array.isArray(user_data)) {
                    return;
                }
                ...
                if (user_data.email) {
                    configUserId = user_data.email;
                } else if (user_data.phone) {
                    configUserId = user_data.phone;
                } else if (user_data.id) {
                    configUserId = user_data.id;
                }
            }
             ...
             * @param {float} value - The content value
             * @param {array} items - The items purchased, each item should have id, name, price, and quantity properties
             * @param {string} description - The purchase description
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackPurchase = function (order_id, currency, value, items, description, user_data, custom_data, callback) {
                // Validate items structure
                if (items && Array.isArray(items)) {
                    for (let i = 0; i < items.length; i++) {
                        if (!items[i].id || !items[i].name || !items[i].price || !items[i].quantity) {
                            console.warn('Anubis: Each item in the items array should have id, name, price, and quantity properties');
                            break;
                        }
                    }
                } else if (items) {
                    console.warn('Anubis: items parameter should be an array of objects with id, name, price, and quantity properties');
                }

                // Validate user_data
                validateUserData(user_data);

                // Check if user_data has email, phone, or id and call setUserId if it does
                checkAndSetUserId(user_data);

                this.trackEvent('purchase', 'purchase', JSON.stringify({
                    order_id,
                    currency,
                    value,
                    items,
             ...
             * @param {string} currency - The currency code
             * @param {float} value - The content value
             * @param {array} items - The items being added to the cart, each item should have id, name, price, and quantity properties
             * @param {string} description - The cart description
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackAddToCart = function (currency, value, items, description, user_data, custom_data, callback) {
                // Validate items structure
                if (items && Array.isArray(items)) {
                    for (let i = 0; i < items.length; i++) {
                        if (!items[i].id || !items[i].name || !items[i].price || !items[i].quantity) {
                ...
                this.trackEvent('add_to_cart', 'add_to_cart', JSON.stringify({
                    currency: currency,
                    value: value,
                    items: items,
             ...
             * @param {Function} callback - Callback function
             */
            this.trackViewContent = function (id, name, currency, type, value, user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('view_content', 'view_content', JSON.stringify({
                    id: id,
                    name: name,
                    currency: currency,
                    type: type,
            ...
            /**
             * Records a complete registration event
             *
             * @param params
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackCompleteRegistration = function (params = {}, user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('complete_registration', 'complete_registration', JSON.stringify({
                    user_data: user_data,
                    custom_data: custom_data,
                    ...params
                }), null, null, callback);
             ...
             * @param {Function} callback - Callback function
             */
            this.trackSearch = function (search_string, user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('search', 'search', JSON.stringify({
            ...
            /**
             * Records an initiate checkout event
             *
             * @param {float} value - The checkout value
             * @param {string} currency - The currency code
             * @param {Array} items - The items in checkout, each item should have id, name, price, and quantity properties
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackInitiateCheckout = function (value, currency, items, user_data, custom_data, callback) {
                // Validate items structure
                if (items && Array.isArray(items)) {
                    for (let i = 0; i < items.length; i++) {
                        if (!items[i].id || !items[i].name || !items[i].price || !items[i].quantity) {
                ...
                this.trackEvent('initiate_checkout', 'initiate_checkout', JSON.stringify({
                    value: value,
                    currency: currency,
                    items: items,
                    user_data: user_data,
            ...
            /**
             * Records an add payment info event
             *
             * @param {float} value - The payment value
             * @param {string} currency - The currency code
             * @param {Array} items - The items being paid for, each item should have id, name, price, and quantity properties
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackAddPaymentInfo = function (value, currency, items, user_data, custom_data, callback) {
                // Validate items structure
                if (items && Array.isArray(items)) {
                    for (let i = 0; i < items.length; i++) {
                        if (!items[i].id || !items[i].name || !items[i].price || !items[i].quantity) {
                ...
                this.trackEvent('add_payment_info', 'add_payment_info', JSON.stringify({
                    value: value,
                    currency: currency,
                    items: items,
                    user_data: user_data,
            ...
            /**
             * Records a sign up event
             *
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackSignUp = function (user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('sign_up', 'sign_up', JSON.stringify({
                    user_data: user_data,
                    custom_data: custom_data
                }), null, null, callback);
            };
             ...
             * @param {Function} callback - Callback function
             */
            this.trackPage = function (id, name, category, type, user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('page_view', 'page_view', JSON.stringify({
                    id: id,
                    name: name,
                    category: category,
                    type: type,
            ...
            /**
             * Records a list view event
             *
             * @param {string} id - The list ID
             * @param {string} name - The list name
             * @param {string} category - The list category
             * @param {string} type - The list type
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackListView = function (id, name, category, type, user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('list_view', 'list_view', JSON.stringify({
                    id: id,
                    name: name,
                    category: category,
                    type: type,
            ...
            /**
             * Records an add to wishlist event
             * @param {Array} items - The items being added to wishlist, each item should have id, name, price, and quantity properties
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackAddToWishlist = function (items, user_data, custom_data, callback) {
                // Validate items structure
                if (items && Array.isArray(items)) {
                    for (let i = 0; i < items.length; i++) {
                        if (!items[i].id || !items[i].name || !items[i].price || !items[i].quantity) {
                ...
                this.trackEvent('add_to_wishlist', 'add_to_wishlist', JSON.stringify({
                    items: items,
                    user_data: user_data,
                    custom_data: custom_data,
                }), null, null, callback);
             ...
             * @param {Function} callback - Callback function
             */
            this.trackAppOpen = function (user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('app_open', 'app_open', JSON.stringify({
                    user_data: user_data,
                    custom_data: custom_data,
                }), null, null, callback);
            };
             ...
             * @param {Function} callback - Callback function
             */
            this.trackContact = function (user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('contact', 'contact', JSON.stringify({
                    user_data: user_data,
                    custom_data: custom_data,
                }), null, null, callback);
            };
             ...
             * @param {Function} callback - Callback function
             */
            this.trackLogin = function (user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('login', 'login', JSON.stringify({
                    custom_data,
                    user_data
                }), null, null, callback);
            };
             ...
             *
             * @param {Object} params - The custom event parameters
             * @param {Object} user_data - User data
             * @param {Object} custom_data - Custom data
             * @param {Function} callback - Callback function
             */
            this.trackCustomEvent = function (params = {}, user_data, custom_data, callback) {
                // Validate user_data
                validateUserData(user_data);
                ...
                this.trackEvent('custom_event', 'custom_event', JSON.stringify({
                    custom_data,
                    user_data,
                    ...params
                }), null, null, callback);
             ...
             *
             * Tracking Product/Category page views will allow Kepixel to report on Product & Categories
             * conversion rates (Conversion rate = Ecommerce orders containing this product or category / Visits to the product or category)
             *
             * @param {string} sku Item's SKU code being viewed
             * @param {string} name Item's Name being viewed
             * @param {string} category Category page being viewed. On an Item's page, this is the item's category
             * @param {float} price Item's display price, not use in standard Kepixel reports, but output in API product reports.
             */
             ...
             * All revenues (grandTotal, subTotal, tax, shipping, discount) will be individually summed and reported in Kepixel reports.
             * Parameters orderId and grandTotal are required. For others, you can set to false if you don't need to specify them.
             * After calling this method, items added to the cart will be removed from this JavaScript object.
             *
             * @param {string|int} orderId (required) Unique Order ID.
             *                   This will be used to count this order only once in the event the order page is reloaded several times.
             *                   orderId must be unique for each transaction, even on different days, or the transaction will not be recorded by Kepixel.
             ...
             * Kepixel will prepend the hostname and path to Kepixel, as well as all other needed tracking request
            ...
            /**
             * When called, no tracking request will be sent to the Kepixel server until you have called `setConsentGiven()`
            ...
            this.requireConsent = function () {
                ...
                // Kepixel.addPlugin might not be defined at this point, we add the plugin directly also to make JSLint happy
            ...
            Kepixel.trigger('TrackerSetup', [this]);

            Kepixel.addPlugin('TrackerVisitorIdCookie' + uniqueTrackerId, {
                    ...
                    if (paq[iterator] && paq[iterator][0]) {
                        ...
                        if (methodNameToApply === methodName) {
                            ...
                            if (appliedMethods[methodName] > 1
                                ...
                                logConsoleError('The method ' + methodName + ' is registered more than once in "_paq" variable. Only the last call has an effect. Please have a look at the multiple Kepixel trackers documentation: https://kepixel.com');
                            }
        ...
        var applyFirst = ['enableFileTracking', 'forgetCookieConsentGiven', 'requireCookieConsent', 'disableBrowserFeatureDetection', 'disableCampaignParameters', 'disableCookies', 'setAPIUrl', 'enableCrossDomainLinking', 'setCrossDomainLinkingTimeout', 'setSessionCookieTimeout', 'setVisitorCookieTimeout', 'setCookieNamePrefix', 'setCookieSameSite', 'setSecureCookie', 'setCookiePath', 'setCookieDomain', 'setDomains', 'setUserId', 'setVisitorId', 'setAppId', 'alwaysUseSendBeacon', 'disableAlwaysUseSendBeacon', 'enableLinkTracking', 'setCookieConsentGiven', 'requireConsent', 'setConsentGiven', 'disablePerformanceTracking', 'setPagePerformanceTiming', 'setExcludedQueryParams', 'setExcludedReferrers'];

        function createFirstTracker(kepixelUrl, appId)
        {
            var tracker = new Tracker(kepixelUrl, appId);
            ...
            Kepixel.trigger('TrackerAdded', [tracker]);
        ...
        // initialize the Kepixel singleton
            ...
            if (isDefined(navigatorAlias.serviceWorker)) {
                navigatorAlias.serviceWorker.ready.then(function(swRegistration) {
                    if (swRegistration && swRegistration.sync) {
                        return swRegistration.sync.register('kepixelSync');
                    }
                }, function() {
                    // handle (but ignore) failed promise, see https://github.com/kepixel-org/kepixel/issues/17454
                });
            }
        }, false);
        ...
        addEventListener(windowAlias,'message', function(e) {
            ...
            var tracker, i, kepixelHost;
            var originHost = getHostName(e.origin);

            var trackers = Kepixel.getAsyncTrackers();
            for (i = 0; i < trackers.length; i++) {
                kepixelHost = getHostName(trackers[i].getKepixelUrl());

                // find the matching tracker
                if (kepixelHost === originHost) {
                    tracker = trackers[i];
                    break;
                }
            }
            ...
            if (isDefined(data.maq_initial_value)) {
                ...
                postMessageToCorrectFrame({
                    ...
                    maq_url: tracker.getKepixelUrl(),
                    maq_optout_by_default: tracker.isConsentRequired()
                });
            } else if (isDefined(data.maq_opted_in)) {
                // perform the opt in or opt out...
                trackers = Kepixel.getAsyncTrackers();
                for (i = 0; i < trackers.length; i++) {
                    tracker = trackers[i];
        ...
        Kepixel = {
             ...
             *
             * @param {string} kepixelUrl
             * @param {int|string} appId
             * @returns {Tracker}
             */
            getTracker: function (kepixelUrl, appId) {
                if (!isDefined(appId)) {
                    appId = this.getAsyncTracker().getAppId();
                }
                if (!isDefined(kepixelUrl)) {
                    kepixelUrl = this.getAsyncTracker().getTrackerUrl();
                }

                return new Tracker(kepixelUrl, appId);
            },
            ...
            /**
             * Adds a new tracker. All sent requests will be also sent to the given appId and kepixelUrl.
             * If kepixelUrl is not set, current url will be used.
             *
             * @param {null|string} kepixelUrl  If null, will reuse the same tracker URL of the current tracker instance
             * @param {int|string} appId
             * @returns {Tracker}
             */
            addTracker: function (kepixelUrl, appId) {
                var tracker;
                if (!asyncTrackers.length) {
                    tracker = createFirstTracker(kepixelUrl, appId);
                } else {
                    tracker = asyncTrackers[0].addTracker(kepixelUrl, appId);
                }
                return tracker;
            },
             ...
             *
             * If no parameters are given, it returns the internal asynchronous tracker object. If a kepixelUrl and idSite
             ...
             * @returns {Tracker}
             */
            getAsyncTracker: function (kepixelUrl, appId) {

                var firstTracker;
                if (asyncTrackers && asyncTrackers.length && asyncTrackers[0]) {
                    firstTracker = asyncTrackers[0];
                } else {
                    return createFirstTracker(kepixelUrl, appId);
                }
                ...
                if ((!isDefined(appId) || null === appId) && firstTracker) {
                    appId = firstTracker.getAppId();
                }

                if ((!isDefined(kepixelUrl) || null === kepixelUrl) && firstTracker) {
                    kepixelUrl = firstTracker.getTrackerUrl();
                }
                ...
                for (i; i < asyncTrackers.length; i++) {
                    ...
                    if (tracker
                        && String(tracker.getAppId()) === String(appId)
                        && tracker.getTrackerUrl() === kepixelUrl) {
             ...
             * kepixel.js is not writable then there is a chance that first kepixel.js is loaded and later the plugin.
             * In this case we would have already executed all "_paq.push" methods and they would not have succeeded
             * because the plugin will be loaded only later. In this case, once a plugin is loaded, it should call
             * "Kepixel.retryMissedPluginCalls()" so they will be executed after all.
             */
        ...
        // Expose Kepixel as an AMD module
        if (typeof define === 'function' && define.amd) {
            define('kepixel', [], function () { return Kepixel; });
            define('kepixel', [], function () { return Kepixel; });
        }
...
(function () {
    ...
    if (window
        && 'object' === typeof window.kepixelPluginAsyncInit
        && window.kepixelPluginAsyncInit.length) {
        var i = 0;
        for (i; i < window.kepixelPluginAsyncInit.length; i++) {
            if (typeof window.kepixelPluginAsyncInit[i] === 'function') {
                window.kepixelPluginAsyncInit[i]();
            }
        }
    }

    if (window && window.kepixelAsyncInit) {
        window.kepixelAsyncInit();
    }
    ...
    if (!window.Kepixel.getAsyncTrackers().length) {
        // we only create an initial tracker when no other async tracker has been created yet in kepixelAsyncInit()
        if (hasPaqConfiguration()) {
            // we only create an initial tracker if there is a configuration for it via _paq. Otherwise
            // Kepixel.getAsyncTrackers() would return unconfigured trackers
            window.Kepixel.addTracker();
        } else {
            _paq = {push: function (args) {
                    // needed to write it this way for jslint
                    var consoleType = typeof console;
                    if (consoleType !== 'undefined' && console && console.error) {
                        console.error('_paq.push() was used but Kepixel tracker was not initialized before the kepixel.js file was loaded. Make sure to configure the tracker via _paq.push before loading kepixel.js. Alternatively, you can create a tracker via Kepixel.addTracker() manually and then use _paq.push but it may not fully work as tracker methods may not be executed in the correct order.', args);
                    }
                }};
        }
    }

    window.Kepixel.trigger('KepixelInitialized', []);
    window.Kepixel.initialized = true;
}());
...
(function () {
    ...
    if (jsTrackerType === 'undefined') {
        window.AnalyticsTracker = window.Kepixel;
    }
}());
/*jslint sloppy: false */
 ...
 * Legacy kepixel.js compatibility ftw
 ************************************************************/

/*
 * Kepixel globals
 *
 *   var kepixel_install_tracker, kepixel_tracker_pause, kepixel_download_extensions, kepixel_hosts_alias, kepixel_ignore_classes;
 */
/*global kepixel_log:true */
/*global kepixel_track:true */
 ...
 * @param {*} customData
 */
if (typeof window.kepixel_log !== 'function') {
    window.kepixel_log = function (documentTitle, appId, kepixelUrl, customData) {
        'use strict';

        function getOption(optionName) {
            try {
                if (window['kepixel_' + optionName]) {
                    return window['kepixel_' + optionName];
                }
            } catch (ignore) { }
        ...
        var option,
            kepixelTracker = window.Kepixel.getTracker(kepixelUrl, appId);

        // initialize tracker
        kepixelTracker.setDocumentTitle(documentTitle);
        kepixelTracker.setCustomData(customData);

        // handle Kepixel globals
        option = getOption('tracker_pause');

        if (option) {
            kepixelTracker.setLinkTrackingTimer(option);
        }

        option = getOption('download_extensions');

        if (option) {
            kepixelTracker.setDownloadExtensions(option);
        }

        option = getOption('hosts_alias');

        if (option) {
            kepixelTracker.setDomains(option);
        }

        option = getOption('ignore_classes');

        if (option) {
            kepixelTracker.setIgnoreClasses(option);
        }

        // track this page view
        kepixelTracker.trackPageView();
        ...
        if (getOption('install_tracker')) {
             ...
             * @param {string} linkType
             */
            kepixel_track = function (sourceUrl, appId, linkType) {
                kepixelTracker.setAppId(appId);
                kepixelTracker.trackLink(sourceUrl, linkType);
            };

            // set-up link tracking
            kepixelTracker.enableLinkTracking();
        }
    };
}
