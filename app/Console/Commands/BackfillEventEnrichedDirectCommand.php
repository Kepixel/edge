...
/**
 * Direct ClickHouse backfill - NO QUEUE, processes synchronously.
 * Much faster than queue-based approach for large backfills.
 * Uses the same extraction logic as ProcessEventEnrichedJob.
 */
class BackfillEventEnrichedDirectCommand extends Command
{
    protected $signature = 'events:backfill-enriched-direct
                            {--chunk=5000 : Number of rows to process per batch}
    ...
    protected $description = 'Backfill event_enriched directly via ClickHouse INSERT...SELECT (fastest method)';
    ...
    public function handle(): int
    {
        ...
        // Count total
        $total = $this->countTotal($fromDate, $toDate, $teamId, $sourceId, $skipExisting);
        ...
        $this->info("Total rows to process: " . number_format($total));
        ...
        while ($offset < $total) {
            try {
                $rows = $this->fetchBatch($offset, $chunkSize, $fromDate, $toDate, $teamId, $sourceId, $skipExisting);
                ...
                foreach ($rows as $row) {
                    $enriched = $this->enrichEvent($row);
                    if ($enriched) {
                        $enrichedRows[] = $enriched['event'];
                        if ($enriched['identity']) {
                            $identityRows[] = $enriched['identity'];
                        }
                        if ($enriched['profile']) {
                            $profileRows[] = $enriched['profile'];
                        }
                    }
                }
                ...
                if (!empty($enrichedRows)) {
                    $this->insertEnrichedBatch($enrichedRows);
                }
                if (!empty($identityRows)) {
                    $this->insertIdentityBatch($identityRows);
                }
                if (!empty($profileRows)) {
                    $this->insertProfileBatch($profileRows);
                }
                ...
                Log::error('BackfillEventEnrichedDirect: Error', [
                    'offset' => $offset,
                    'error' => $e->getMessage(),
                ]);
                $offset += $chunkSize;
        ...
        $this->table(['Metric', 'Count'], [
            ...
            ['Failed batches', number_format($failed)],
        ]);

        return self::SUCCESS;
    }

    private function countTotal(?string $fromDate, ?string $toDate, ?string $teamId, ?string $sourceId, bool $skipExisting): int
    {
        ...
        if ($skipExisting) {
            $sql = "
                SELECT count() as cnt
                FROM event_upload_logs e
                {$where}
                AND e.message_id NOT IN (SELECT message_id FROM event_enriched WHERE message_id != '')
            ";
        }
    ...
    private function enrichEvent(array $row): ?array
    {
        ...
        $properties = is_string($row['properties'] ?? null)
        ...
        // Identity mapping
        if ($eventType === 'identify' && !empty($userId) && !empty($anonymousId)) {
            $result['identity'] = [
                $teamId,
                $anonymousId,
                $userId,
